...
1. [instrsarm64sve.h](https://github.com/kunalspathak/SveEncodingFiles/blob/main/instrsarm64_sve.h): All the instructions that RyuJIT offers are stored in a table. Each instruction can have several different formats by which they operate, depending on the size of vector, size of data, size of individual elements, those containing immediate operand and so forth. For e.g., `ld1sb` instruction's entry in `Fig 1.` has 6 different formats (as seen in [1](https://developer.arm.com/documentation/ddi0602/2022-03/SVE-Instructions/LD1SB--scalar-plus-immediate---Contiguous-load-signed-bytes-to-vector--immediate-index--), [2](https://developer.arm.com/documentation/ddi0602/2022-03/SVE-Instructions/LD1SB--scalar-plus-scalar---Contiguous-load-signed-bytes-to-vector--scalar-index--), [3](https://developer.arm.com/documentation/ddi0602/2022-03/SVE-Instructions/LD1SB--scalar-plus-vector---Gather-load-signed-bytes-to-vector--vector-index--) and [4](https://developer.arm.com/documentation/ddi0602/2022-03/SVE-Instructions/LD1SB--vector-plus-immediate---Gather-load-signed-bytes-to-vector--immediate-index--)). Every instruction format has the normalized opcodes in binary and hexadecimal formats. In the first instruction format of `Fig. 1`, the binary representation comes from [the Arm manual](https://developer.arm.com/documentation/ddi0602/2022-03/SVE-Instructions/LD1B--scalar-plus-vector---Gather-load-unsigned-bytes-to-vector--vector-index--) seen in `Fig 2.`. The normalized version in our codebase basically has `0` at all the bit positions that encodes the information during code generation. One of the example is registers used in an instruction. If you see the binary representation, there are characters like `g`, `n` and `t` in them. Those positions are filled up when register corresponding to `g`, `n` and `t` are known during code generation. Without the tool, it would have been very time consuming task to translate the information in Arm manual (from `Fig 2.`) into the C++ code (in `Fig 1.`). 
...
The future of Scalable Vector Extension (SVE) in .NET looks promising, particularly with the introduction of streaming mode SVE and Scalable Matrix Extension (SME). These advancements are set to enhance performance and efficiency, especially for workloads that benefit from parallel processing and high throughput. However, they come with entire set of different complications that we will have to evaluate. SME introduces a concept of Scalable Vector Length (SVL) which is different than the VL that I have been discussing in this blog. When they turn on "streaming mode", their application code runs on SVL and continue doing so until streaming is turned off, after which it executes on VL. While in streaming mode, two things take place that are worth mentioning.

A. Most of the SVE/SVE2 instructions and NEON instructions becomes invalid. All the .NET APIs that expose such instructions will start faulting and we have a design challenge ahead of us to see how developers do not accidently write their code such that their application start throwing faults when streaming is turned ON at some other place in the code. For example, suppose there is a third-party library that invokes .NET APIs which generate NEON/SVE code as shown in `LibraryMethod()`. Customer code that uses that library can see different behavior, depending on streaming was turned on or not. `CustomerCode1()` will execute just fine, but `CustomerCode2()` will throw exception. We want to mitigate such problems without having that responsibility on developer's shoulders. Either we can have the .NET APIs that expose streaming compatible instructions in a separate namespace for developer to easily spot these things or have a method annotation on such APIs, or some other method.
...
B. SME introduced `ZA` matrix array register to store 2D data. We will have to add support of this new register in our register allocator. It also has some interesting rules on saving and restoring the `ZA` state to "lazy state buffer" depending on that state of `ZA` - off, active and dormant. When switching out of streaming mode, all the vector and predicate registers are zeroed outand that means we need to store the contents of all those registers and restore them back, which can add significant overhead.
