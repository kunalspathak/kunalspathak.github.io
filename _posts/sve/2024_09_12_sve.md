...
It acts as a ternary logical operator in which depending on the values of predicate register `Pv`, for active lanes, it will pick the elements from `Zn` and for inactive lanes, from `Zm` and merge them in destination `Zd`. Essentially, it is doing the operation `Zd = Pv ? Zn : Zm`.
...
This API is one of the backbone of other .NET SVE APIs that we implemented and is the main reason of having less explosion of number of APIs. To see why, let us dive in to check various types of instructions SVE offers.
...
The `svabs_s32_m` API with `_m` in the end calculates the Absolute value of elements that are active. For inactive lanes, pick the corresponding lanes from `inactive` parameter and store them in the destination.

```c#
val_m = ConditionalSelect(Pg, Abs(op1), inactive);
val_x = ConditionalSelect(Pg, Abs(op1), val_x);
val_z = ConditionalSelect(Pg, Abs(op1), Zero);
```

`Abs(op1)` can be represented as `ConditionalSelect(True, Abs(op1), Zero)`


There are [21 ACLE APIs for Abs](https://developer.arm.com/architectures/instruction-sets/intrinsics/#f:@navigationhierarchiessimdisa=[sve]&q=svabs[), but we were able to have just 6 of them, each for different signed data types.
...
#### Predicated and Unpredicated instructions
...
`add` example - https://developer.arm.com/architectures/instruction-sets/intrinsics/#f:@navigationhierarchiessimdisa=[sve]&f:@navigationhierarchiesreturnbasetype=[int,uint]&q=svadd[_s8
3 C++ APIs vs. 1 C# API (both predicated and unpredicated)
 
 `compact` example - https://developer.arm.com/architectures/instruction-sets/intrinsics/#f:@navigationhierarchiessimdisa=[sve]&q=svcompact (explicit predicated)

 `zip2` example - https://developer.arm.com/architectures/instruction-sets/intrinsics/#f:@navigationhierarchiessimdisa=[sve]&q=svzip2[_u8] (only unpredicated)

 6 C++ APIs vs. 6 C# APIs


The full list of issues is:

TODO: Links to Alan's issues 
...
Predicated version, Unpredicated version and both
